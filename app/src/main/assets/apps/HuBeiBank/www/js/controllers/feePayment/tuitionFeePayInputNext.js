					...
					if(tutionInfo.length>0){
						...
						for(var i = 0;i < tutionInfo.length;i++){
							...
							htmlStr +='<li class="mui-table-view-cell">'
								...
								+'<label>￥'+amt+'</label>'
								+'<input name="checkbox1" value="'+tutionInfo[i].Amt+'" type="checkbox" FeeNo="'+tutionInfo[i].FeeNo+'" FeeNam="'+tutionInfo[i].FeeNam+'">'
								+'</div></li>';
						}
		...
		function checkAll(){
	        ...
	        mui(".content_BOXall").on("change","input[name='checkbox1']",function(){ 
	                ...
	                { 
	                    ...
	                    if(this.value){
	                    	tempA +=parseFloat(this.value);
	                    	document.getElementById("moneyAllSpan").innerText = "￥"+format.formatMoney(tempA, 2);
	                    }else{
	                    	tempA += 0;
	                    	document.getElementById("moneyAllSpan").innerText = "￥"+format.formatMoney(tempA, 2);
	                    }
